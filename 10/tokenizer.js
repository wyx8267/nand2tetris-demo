...
function JackTokenizer(data, fileName) {
  ...
  this.outputPath = fileName + "X.xml";
  // 单行注释
  this.notesRe1 = /^(\/\/)/;
  /* */
  this.notesRe2 = /^(\/\*).*(\*\/)$/;
...
JackTokenizer.prototype = {
  ...
  _isValidStr(str) {
    if (
      this.notesRe1.test(str) ||
      this.notesRe2.test(str) ||
      this.notesRe3.test(str)
    ) {
      return false;
  ...
  _stringVal(token) {
    token = token.replace(this.strRe1, "").replace(this.strRe2, "");
